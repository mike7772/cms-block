...
this package's own TypeScript build tooling. `npm run prepare` rebuilds it locally.
...
## Known follow-up (not yet addressed by this port)

Blocks were authored against portal-frontend's Tailwind v4 theme (custom tokens like
`text-ink/70`, `bg-sky-pale`). Both consuming apps (OCCMS_FRONTED_V1, PUBLIC_PORTAL) are
on Tailwind v3 — those custom color tokens don't exist there yet and will need adding to
each app's Tailwind theme (or the blocks' classes normalized) before styling matches.
