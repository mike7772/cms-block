...
function withIds(items: NestedItem[], prefix: string): NestedItem[] {
  return items.map((item, index) => ({
    ...item,
    props: {
      ...item.props,
      id: (item.props.id as string) || `${prefix}-${item.type}-${index}`,
    },
  }));
}
...
export const MENU_DEFAULT_LINKS = [
  {
    ...
    openInNewTab: false,
    isButton: false,
    groups: [] as Array<typeof MENU_DEFAULT_GROUP>,
  },
  {
    label: "Product",
    url: "/#product",
...
export function buildModelHeaderData(): Data {
  return {
    root: { props: {} },
    content: [
      {
...
export function buildModelFooterData(): Data {
  return {
    root: { props: {} },
    content: [
      {
