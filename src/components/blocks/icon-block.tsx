...
const PATHS: Record<string, string> = {
  ...
  heart:
    "M12 21s-7-4.5-9.5-8C.5 9.5 3 5 7 5c2 0 3.5 1 5 3 1.5-2 3-3 5-3 4 0 6.5 4.5 4.5 8C19 16.5 12 21 12 21z",
...
export default function IconBlock({ block }: { block: IconBlockType }) {
  const size = sizeClass[block.size ?? "medium"] ?? sizeClass.medium;
  const color = block.color || "#1B4332";
  const name = block.iconName ?? "star";
  ...
  const wrapped = block.linkUrl ? (
    <Link href={block.linkUrl} className="inline-flex">
      {glyph}
    </Link>
  ) : (
    glyph
  ...
  return (
    <div className={`flex ${alignClass[block.align ?? "center"]}`}>{wrapped}</div>
  );
}
