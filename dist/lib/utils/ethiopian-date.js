...
export function toGregorian(etYear, etMonth, etDay) {
    ...
    const jdn = ETHIOPIAN_EPOCH + 365 * (etYear - 1) + Math.floor(etYear / 4) + 30 * (etMonth - 1) + etDay - 1;
 ...
 */
export function toEthiopian(gregYear, gregMonth, gregDay) {
 ...
 */
export function getDaysInMonth(etYear, etMonth) {
    if (etMonth <= 12) {
        return 30;
    }
    else {
 ...
 */
export function formatEthiopianDate(date, language = 'en', format = 'long') {
    if (!date)
        return 'Date not set';
    try {
        const dateObj = typeof date === 'string' ? new Date(date) : date;
        const [etYear, etMonth, etDay] = ethiopianDate.toEthiopian(dateObj.getFullYear(), dateObj.getMonth() + 1, dateObj.getDate());
        if (format === 'numeric') {
 ...
 * @returns Date object
 */
export function ethiopianToGregorianDate(etDateStr) {
 ...
 */
export function gregorianToEthiopianString(date) {
